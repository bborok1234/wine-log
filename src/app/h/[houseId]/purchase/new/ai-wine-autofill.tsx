...
export function AiWineAutofill({ initialText }: { initialText: string }) {
  ...
  function applyParsed(parsed: ParsedWine) {
    ...
    if (typeof parsed.vintage === "number") setInputValue(vintageId, String(parsed.vintage));
  ...
  async function handleAutoFillFromText() {
    ...
    try {
      ...
      setErrorMessage(e instanceof Error ? e.message : "AI 분석에 실패했습니다.");
    } finally {
      setIsParsing(false);
    }
  }
  ...
  async function handlePickImage(file: File) {
    ...
    try {
      ...
      const data = await postJson<ParsedWine>("/api/ai/analyze-label", { base64Data, mimeType });
      applyParsed(data);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : "라벨 분석에 실패했습니다.");
    } finally {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-lg shadow-indigo-100 group">
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-black/30 group-hover:bg-black/40 transition-colors">
            <button
              ...
              className="bg-white/90 backdrop-blur text-stone-800 px-4 py-2 rounded-full font-bold text-sm flex items-center gap-2 hover:bg-white transition-colors"
            >
      ...
      {errorMessage ? (
        <div className="text-xs font-bold text-rose-600 px-1">{errorMessage}</div>
      ) : null}

      <div className="sr-only">
        {/* ids for server-rendered fields */}
        <input id={producerId} />
        <input id={nameId} />
        <input id={vintageId} />
        <input id={countryId} />
